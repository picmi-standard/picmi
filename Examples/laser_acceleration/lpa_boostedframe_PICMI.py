...
gamma_boost = 10.0  # Lorentz factor of the boosted-frame

# Physics parameters
# ------------------

# --- laser
laser_a0 = 1.0  # Normalized potential vector
laser_wavelength = 8e-07  # Wavelength of the laser (in meters)
laser_waist = 8e-06  # Waist of the laser (in meters)
laser_length = 3.0e-6  # Length of the laser (in meters)
laser_z0 = -6.0e-6  # Initial position of the centroid (meters)
laser_focal_position = 0.0  # Laser focal position (meters)
laser_antenna_z = -0.1e-6  # Position of the antenna (meters)

# --- plasma
plasma_density = 2.5e25
plasma_min = [-25.0e-6, -25.0e-6, 5.0e-6]
plasma_max = [25.0e-6, 25.0e-6, 3000.0e-6]
...
zmin = -15.0e-6
zmax = +5.0e-6
moving_window_velocity = [0.0, 0.0, cst.c]
...
laser = picmi.GaussianLaser(
    ...
    focal_position=[0.0, 0.0, laser_focal_position],
    centroid_position=[0.0, 0.0, laser_z0],
...
uniform_plasma = picmi.UniformDistribution(
    density=plasma_density, lower_bound=plasma_min, upper_bound=plasma_max, fill_in=True
)
electrons = picmi.Species(
    particle_type="electron", particle_shape="cubic", name="electrons", initial_distribution=uniform_plasma
)
ions = picmi.Species(
    particle_type="H", charge_state=+1, particle_shape="cubic", name="ions", initial_distribution=uniform_plasma
)
...
beam_dist = picmi.AnalyticDistribution(
    ...
    directed_velocity=[0.0, 0.0, beam_uz * cst.c],
)
beam = picmi.Species(particle_type="electron", particle_shape="cubic", name="beam", initial_distribution=beam_dist)
...
smoother = picmi.BinomialSmoother(n_pass=[1, 1, 1], compensation=[False, False, False])
galilean_velocity = [0, 0, -cst.c * (1.0 - 1.0 / gamma_boost**2) ** 0.5]
solver = picmi.ElectromagneticSolver(
    grid=grid, method=em_solver_method, cfl=1.0, source_smoother=smoother, galilean_velocity=galilean_velocity
)

# Diagnostics
# -----------
field_diag = picmi.FieldDiagnostic(grid=grid, period=100, write_dir="diags")

part_diag = picmi.ParticleDiagnostic(period=100, species=[electrons, ions, beam], write_dir="diags")

field_diag_lab = picmi.LabFrameFieldDiagnostic(
    grid=grid,
    num_snapshots=20,
...
part_diag_lab = picmi.LabFrameParticleDiagnostic(
    grid=grid,
    num_snapshots=20,
    dt_snapshots=0.5 * (zmax - zmin) / cst.c,
...
sim = picmi.Simulation(solver=solver, max_steps=max_steps, gamma_boost=gamma_boost, verbose=1)

sim.add_species(
    electrons, layout=picmi.GriddedLayout(grid=grid, n_macroparticle_per_cell=plasma_number_per_cell_each_dim)
)
sim.add_species(ions, layout=picmi.GriddedLayout(grid=grid, n_macroparticle_per_cell=plasma_number_per_cell_each_dim))
sim.add_species(
    beam,
    layout=picmi.GriddedLayout(grid=grid, n_macroparticle_per_cell=beam_number_per_cell_each_dim),
    initialize_self_field=True,
)

laser_antenna = picmi.LaserAntenna(
    position=[0.0, 0.0, laser_antenna_z],  # This point is on the laser plane

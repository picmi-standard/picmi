...
laser_a0 = 4.0  # Normalized potential vector
...
laser_injection_loc = 9.0e-6  # Position of injection (in meters, along z)
laser_focal_distance = 100.0e-6  # Focal distance from the injection (in meters)
laser_t_peak = 30.0e-15  # The time at which the laser reaches its peak
# at the antenna injection location (in seconds)
# --- plasma
plasma_density_expression = "1.e23*(1+tanh((z - 20.e-6)/10.e-6))/2."
plasma_min = [-20.0e-6, -20.0e-6, 0.0e-6]
plasma_max = [20.0e-6, 20.0e-6, 1.0e-3]

# --- electron bunch
bunch_physical_particles = 1.0e8
bunch_rms_size = [1.0e-6, 1.0e-6, 1.0e-6]
bunch_rms_velocity = [0.0, 0.0, 10.0 * cst.c]
bunch_centroid_position = [0.0, 0.0, -22.0e-6]
bunch_centroid_velocity = [0.0, 0.0, 1000.0 * cst.c]
...
laser = picmi.GaussianLaser(
    ...
    focal_position=[0.0, 0.0, laser_focal_distance + laser_injection_loc],
    centroid_position=[0.0, 0.0, laser_injection_loc - cst.c * laser_t_peak],
    polarization_direction=[math.cos(laser_polarization), math.sin(laser_polarization), 0.0],
...
plasma_dist = picmi.AnalyticDistribution(
    density_expression=plasma_density_expression, lower_bound=plasma_min, upper_bound=plasma_max, fill_in=True
)
...
solver = picmi.ElectromagneticSolver(grid=grid, cfl=1.0, method=em_solver_method, source_smoother=smoother)

# Diagnostics
# -----------
field_diag = picmi.FieldDiagnostic(name="diag1", grid=grid, period=100, warpx_plot_finepatch=1, warpx_plot_crsepatch=1)
...
antenna = picmi.LaserAntenna(position=(0, 0, 9.0e-6), normal_vector=(0, 0, 1.0))
sim.add_laser(laser, injection_method=antenna)

# Add the plasma: continuously injected by the moving window
plasma_layout = picmi.GriddedLayout(grid=grid, n_macroparticle_per_cell=n_macroparticle_per_cell)
sim.add_species(species=plasma, layout=plasma_layout)

# Add the beam
beam_layout = picmi.PseudoRandomLayout(n_macroparticles=10**5, seed=0)
initialize_self_field = True
if picmi.codename == "warpx":
    initialize_self_field = False
sim.add_species(species=beam, layout=beam_layout, initialize_self_field=initialize_self_field)

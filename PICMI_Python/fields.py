...
class PICMI_ElectromagneticSolver(_ClassWithInit):
    ...
    def __init__(
        ...
        **kw,
        ...
        assert method is None or method in PICMI_ElectromagneticSolver.methods_list, Exception(
            "method must be one of " + ", ".join(PICMI_ElectromagneticSolver.methods_list)
        )
...
class PICMI_ElectrostaticSolver(_ClassWithInit):
    ...
    def __init__(self, grid, method=None, required_precision=None, maximum_iterations=None, **kw):

        assert method is None or method in PICMI_ElectrostaticSolver.methods_list, Exception(
            "method must be one of " + ", ".join(PICMI_ElectrostaticSolver.methods_list)
        )
...
class PICMI_MagnetostaticSolver(_ClassWithInit):
    ...
    def __init__(self, grid, method=None, **kw):
        ...
        assert method is None or method in PICMI_MagnetostaticSolver.methods_list, Exception(
            "method must be one of " + ", ".join(PICMI_MagnetostaticSolver.methods_list)
        )
...
class PICMI_Cartesian1DGrid(_ClassWithInit):
    ...
    * R. Lehe, "Electromagnetic wave propagation in Particle-In-Cell codes," 2016,
    ...
    def __init__(
        ...
        lower_bound=None,
        upper_bound=None,
        lower_boundary_conditions=None,
        upper_boundary_conditions=None,
        nx=None,
        xmin=None,
        xmax=None,
        bc_xmin=None,
        ...
        refined_regions=[],
        lower_bound_particles=None,
        upper_bound_particles=None,
        xmin_particles=None,
        xmax_particles=None,
        lower_boundary_conditions_particles=None,
        upper_boundary_conditions_particles=None,
        bc_xmin_particles=None,
        bc_xmax_particles=None,
        guard_cells=None,
        pml_cells=None,
        **kw,
    ):
        ...
        assert (number_of_cells is None) and (nx is not None) or (number_of_cells is not None) and (nx is None), (
            Exception("Either number_of_cells or nx must be specified")
        )
        assert (lower_bound is None) and (xmin is not None) or (lower_bound is not None) and (xmin is None), Exception(
            "Either lower_bound or xmin must be specified"
        )
        assert (upper_bound is None) and (xmax is not None) or (upper_bound is not None) and (xmax is None), Exception(
            "Either upper_bound or xmax must be specified"
        )
        assert (
            (lower_boundary_conditions is None)
            and (bc_xmin is not None)
            or (lower_boundary_conditions is not None)
            and (bc_xmin is None)
        ), Exception("Either lower_boundary_conditions or bc_xmin")
        assert (
            (upper_boundary_conditions is None)
            and (bc_xmax is not None)
            or (upper_boundary_conditions is not None)
            and (bc_xmax is None)
        ...
        assert len(lower_boundary_conditions_particles) == 1, Exception(
            "Wrong number of lower particle boundary conditions specified"
        )
        assert len(upper_boundary_conditions_particles) == 1, Exception(
        ...
        for region in self.refined_regions:
            ...
            assert len(region[3]) == 1, Exception(
...
class PICMI_CylindricalGrid(_ClassWithInit):
    ...
    * R. Lehe, "Electromagnetic wave propagation in Particle-In-Cell codes," 2016,
    ...
    def __init__(
        ...
        lower_boundary_conditions_particles=None,
        upper_boundary_conditions_particles=None,
        bc_rmin_particles=None,
        bc_rmax_particles=None,
        bc_zmin_particles=None,
        bc_zmax_particles=None,
        guard_cells=None,
        pml_cells=None,
        **kw,
    ):
        ...
        # Sanity check and init of input arguments related to grid parameters
        assert (
            (number_of_cells is None)
            and (nr is not None and nz is not None)
            or (number_of_cells is not None)
            and (nr is None and nz is None)
        ), Exception("Either number_of_cells or nr and nz must be specified")
        assert (
            (lower_bound is None)
            and (rmin is not None and zmin is not None)
            or (lower_bound is not None)
            and (rmin is None and zmin is None)
        ), Exception("Either lower_bound or rmin and zmin must be specified")
        assert (
            (upper_bound is None)
            and (rmax is not None and zmax is not None)
            or (upper_bound is not None)
            and (rmax is None and zmax is None)
        ...
        assert (
            (lower_boundary_conditions is None)
            and (bc_zmin is not None)
            or (lower_boundary_conditions is not None)
            and (bc_rmin is None and bc_zmin is None)
        ), Exception("Either lower_boundary_conditions or bc_rmin and bc_zmin must be specified")
        assert (
            (upper_boundary_conditions is None)
            and (bc_rmax is not None and bc_zmax is not None)
            or (upper_boundary_conditions is not None)
            and (bc_rmax is None and bc_zmax is None)
        ...
        for region in self.refined_regions:
            ...
            assert len(region[3]) == 2, Exception(
                "The refinement factor of the refined region must be a vector of length 2"
            )
...
class PICMI_Cartesian2DGrid(_ClassWithInit):
    ...
    * R. Lehe, "Electromagnetic wave propagation in Particle-In-Cell codes," 2016,
    ...
    def __init__(
        ...
        bc_xmin_particles=None,
        bc_xmax_particles=None,
        bc_ymin_particles=None,
        bc_ymax_particles=None,
        guard_cells=None,
        pml_cells=None,
        **kw,
    ):
        ...
        assert (
            ...
            or (number_of_cells is not None)
            and (nx is None and ny is None)
        ), Exception("Either number_of_cells or nx and ny must be specified")
        assert (
            (lower_bound is None)
            and (xmin is not None and ymin is not None)
            or (lower_bound is not None)
            and (xmin is None and ymin is None)
        ...
        assert (
            (lower_boundary_conditions is None)
            and (bc_xmin is not None and bc_ymin is not None)
            or (lower_boundary_conditions is not None)
            and (bc_xmin is None and bc_ymin is None)
        ), Exception("Either lower_boundary_conditions or bc_xmin and bc_ymin must be specified")
        assert (
            (upper_boundary_conditions is None)
            and (bc_xmax is not None and bc_ymax is not None)
            or (upper_boundary_conditions is not None)
            and (bc_xmax is None and bc_ymax is None)
        ...
        assert len(lower_boundary_conditions_particles) == 2, Exception(
            "Wrong number of lower particle boundary conditions specified"
        )
        assert len(upper_boundary_conditions_particles) == 2, Exception(
            "Wrong number of upper particle boundary conditions specified"
        )
    ...
    * R. Lehe, "Electromagnetic wave propagation in Particle-In-Cell codes," 2016,
    ...
    def __init__(
        ...
        **kw,
        ...
        assert (
            (number_of_cells is None)
            and (nx is not None and ny is not None and nz is not None)
            or (number_of_cells is not None)
            and (nx is None and ny is None and nz is None)
        ), Exception("Either number_of_cells or nx, ny, and nz must be specified")
        assert (
            (lower_bound is None)
            and (xmin is not None and ymin is not None and zmin is not None)
            or (lower_bound is not None)
            and (xmin is None and ymin is None and zmin is None)
        ), Exception("Either lower_bound or xmin, ymin, and zmin must be specified")
        assert (
            (upper_bound is None)
            and (xmax is not None and ymax is not None and zmax is not None)
            or (upper_bound is not None)
            and (xmax is None and ymax is None and zmax is None)
        ), Exception("Either upper_bound or xmax, ymax, and zmax must be specified")
        assert (
            (lower_boundary_conditions is None)
            and (bc_xmin is not None and bc_ymin is not None and bc_zmin is not None)
            or (lower_boundary_conditions is not None)
            and (bc_xmin is None and bc_ymin is None and bc_zmin is None)
        ), Exception("Either lower_boundary_conditions or bc_xmin, bc_ymin, and bc_zmin must be specified")
        assert (
            (upper_boundary_conditions is None)
            and (bc_xmax is not None and bc_ymax is not None and bc_zmax is not None)
            or (upper_boundary_conditions is not None)
            and (bc_xmax is None and bc_ymax is None and bc_zmax is None)
        ...
        assert len(lower_boundary_conditions_particles) == 3, Exception(
            "Wrong number of particle lower boundary conditions specified"
        )
        assert len(upper_boundary_conditions_particles) == 3, Exception(
        ...
        for region in self.refined_regions:
            ...
            assert len(region[3]) == 3, Exception(
